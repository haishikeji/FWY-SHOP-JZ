  ...
  <div class="knack-list-page">
	...
	<div class="filter-condition">
        <div class="condition-item">
            <span class="text">窍门名称: </span>
            <Input v-model="table.queryFilter.knackName" placeholder="请输入窍门名称" style="width: 190px;"></Input>
        </div>
        <!-- <div class="condition-item">
		...
		<div class="condition-item" style="position: absolute; top: 0px; right: 0px;">
			<Button type="primary" @click="$router.push({ name: 'knackAdd' })">新增窍门</Button>
		</div>
    </div>
	...
	<div style="margin-top: 20px;">
        <Table border ref="selection" :columns="table.cols" :data="table.datas">
            <template slot-scope="{ row, index }" slot="action">
                <Button type="primary" size="small" @click="$router.push({ name: 'knackEdit', params: { id: row.id } })">修改</Button>
...
<script>
	...
	export default {
	  ...
	  methods: {
    	...
    	fetchDataList() {
    		knackAPI.fetchList(this.table.queryFilter).then(res => {
                if (res.code == 0) {
         ...
         updateShiftStatus(event,id){
            knackAPI.knackShift({"id":id,"shiftStatus":event}).then(res => {
                if (res.code == 0) {
        ...
        remove() {
            ...
            knackAPI.knackDel({ id: this.dialog.delete.id }).then(res => {
                if (res.code == 0) {
