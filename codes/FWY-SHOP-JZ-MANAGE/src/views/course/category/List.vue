...
<script>
	...
	export default {
	  ...
	  methods: {
    	fetchDataTree() {
    		courseAPI.fetchTypeTree().then(res => {
    			if (res.code == 0) {
    	...
    	fetchDetail(id) {
    		courseAPI.fetchTypeDetail({ id: id }).then(res => {
    			if (res.code == 0) {
    				let _group = [];
        ...
        remove () {
    		...
    		courseAPI.courseTypeDel({ id: this.dialog.delete.id }).then(res => {
    			if (res.code == 0) {
        ...
        handleSubmit(name) {
            ...
            this.$refs[name].validate((valid) => {
                if (valid) {
                	let apiOper = this.oprType == 'add' ? courseAPI.courseTypeSave : courseAPI.courseTypeModify;
