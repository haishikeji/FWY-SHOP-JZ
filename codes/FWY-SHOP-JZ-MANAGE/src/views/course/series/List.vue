<template>
  <div class="course-series-list-page">
	...
	<div class="filter-condition">
        <div class="condition-item">
            ...
            <Input v-model="table.queryFilter.courseName" placeholder="请输入系列名称" style="width: 190px;"></Input>
        </div>
        <!-- <div class="condition-item">
		...
		<div class="condition-item" style="position: absolute; top: 0px; right: 0px;">
			<Button type="primary" @click="$router.push({ name: 'courseSeriesAdd' })">新增线上系列</Button>
		</div>
    </div>
	...
	<div style="margin-top: 20px;">
        <Table border ref="selection" :columns="table.cols" :data="table.datas">
            <template slot-scope="{ row, index }" slot="action">
                <Button type="primary" size="small" @click="$router.push({ name: 'courseSeriesEdit', params: { id: row.id } })">修改</Button>
                <Button type="error" size="small" style="margin-left: 10px;" @click="openDeleteDialog(row.id)">删除</Button>
                <Button size="small" style="margin-left: 10px;" @click="$router.push({ name: 'courseSeriesEnroll', params: { id: row.id } })">报名学员</Button>
            </template>
        </Table>
...
<script>
	...
	export default {
                ...
                cols: [
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            ...
                            return h('span', (_value ? _value.length : 0) + '人');
                        }
                    },
                    {
                        title: '上架状态',
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            ...
                            return h('span', {
                                ...
                                on: {
                                    'click': () => {
                                        this.$router.push({ name: 'courseSeriesConfig', params: { id: params.row.id } })
                                    }
                                }
                            }, _text);
                        }
	  ...
	  methods: {
    	...
    	fetchDataList() {
    		courseAPI.fetchSeriesList(this.table.queryFilter).then(res => {
                if (res.code == 0) {
        ...
        updateShiftStatus(event,id){
            courseAPI.courseSeriesShift({"id":id,"shiftStatus":event}).then(res => {
                if (res.code == 0) {
        ...
        remove() {
            ...
            courseAPI.courseSeriesDel({ id: this.dialog.delete.id }).then(res => {
                if (res.code == 0) {
