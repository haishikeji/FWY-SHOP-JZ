<template>
  <div class="activity-series-page">
	...
	<div class="filter-condition">
        <div class="condition-item">
            <span class="text">系列活动名称: </span>
            <Input v-model="table.queryFilter.seriesName" placeholder="请输入活动名称" style="width: 190px;"></Input>
        </div>
        <div class="condition-item">
		...
		<div class="condition-item" style="position: absolute; top: 0px; right: 0px;">
			<Button type="primary" @click="$router.push({ name: 'activitySeriesAdd' })">新增系列活动</Button>
		</div>
    </div>
	...
	<div style="margin-top: 20px;">
        <Table border ref="selection" :columns="table.cols" :data="table.datas">
            <template slot-scope="{ row, index }" slot="action">
                <Button type="primary" size="small" @click="$router.push({ name: 'activitySeriesEdit', params: { id: row.id } })">修改</Button>
            </template>
        </Table>
	</div>
...
<script>
	...
	export default {
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            return h('span', {
                                ...
                                on: {
                                    'click': () => {
                                        this.$router.push({ name: 'activitySeriesConfig', params: { id: params.row.id } })
                                    }
                                }
                            }, '活动配置');
                        }
	  ...
	  methods: {
    	...
    	fetchDataList() {
    		activityAPI.fetchSeriesList(this.table.queryFilter).then(res => {
                if (res.code == 0) {
