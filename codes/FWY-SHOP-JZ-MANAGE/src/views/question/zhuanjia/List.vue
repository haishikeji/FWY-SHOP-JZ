  ...
  <div class="case-list-page">
	...
	<div class="filter-condition">
        <div class="condition-item">
            ...
            <Input v-model="table.queryFilter.name" placeholder="请输入问题" style="width: 190px;"></Input>
        </div>
        <!-- <div class="condition-item">
		...
		<div class="condition-item" style="position: absolute; top: 0px; right: 0px;">
			<Button type="primary" @click="$router.push({ name: 'zhuanjiaQuestionAdd' })">新增问题</Button>
		</div>
    </div>
	...
	<div style="margin-top: 20px;">
        <Table border ref="selection" :columns="table.cols" :data="table.datas">
            <template slot-scope="{ row, index }" slot="action">
                <Button type="primary" size="small" @click="$router.push({ name: 'zhuanjiaQuestionEdit', params: { id: row.id } })">修改</Button>
...
<script>
	...
	export default {
                    ...
                    },
                     {
                        ...
                        render: (h, params) => {
                            ...
                            return h('span', {
                                ...
                                on: {
                                    'click': () => {
                                        this.$router.push({ name: 'zhuanjiaQuestionAnswerList', params: { id: params.row.id } })
                                    }
                                }
                            }, _text);
                        }
	  ...
	  methods: {
    	...
    	fetchDataList() {
    		questionAPI.fetchZhuanjiaQuestionList(this.table.queryFilter).then(res => {
                if (res.code == 0) {
         ...
         updateShiftStatus(event,id){
            questionAPI.zhuanjiaQuestionShift({"id":id,"shiftStatus":event}).then(res => {
                if (res.code == 0) {
        ...
        remove() {
            ...
            questionAPI.zhuanjiaQuestionDel({ id: this.dialog.delete.id }).then(res => {
                if (res.code == 0) {
