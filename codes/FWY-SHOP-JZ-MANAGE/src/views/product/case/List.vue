  ...
  <div class="case-list-page">
	...
	<div class="filter-condition">
        <div class="condition-item">
            <span class="text">案例名称: </span>
            <Input v-model="table.queryFilter.projectName" placeholder="请输入案例名称" style="width: 190px;"></Input>
        </div>
        <!-- <div class="condition-item">
		...
		<div class="condition-item" style="position: absolute; top: 0px; right: 0px;">
			<Button type="primary" @click="$router.push({ name: 'productCaseAdd' })">新增案例</Button>
		</div>
    </div>
	...
	<div style="margin-top: 20px;">
        <Table border ref="selection" :columns="table.cols" :data="table.datas">
            <template slot-scope="{ row, index }" slot="action">
                <Button type="primary" size="small" @click="$router.push({ name: 'productCaseEdit', params: { id: row.id } })">修改</Button>
...
<script>
	import { productAPI,businessAPI  } from '@/api';
	...
	export default {
                    ...
                    },
                    {
                        ...
                        render: (h, params) => {
                            ...
                            return h('span', {
                                ...
                                on: {
                                    'click': () => {
                                        this.$router.push({ name: 'productCaseConfig', params: { id: params.row.id } })
                                    }
                                }
                            }, _text);
                        }
	  ...
	  methods: {
    	...
    	fetchDataList() {
    		productAPI.fetchCaseList(this.table.queryFilter).then(res => {
                if (res.code == 0) {
         ...
         updateShiftStatus(event,id){
            productAPI.caseShift({"id":id,"shiftStatus":event}).then(res => {
                if (res.code == 0) {
        ...
        remove() {
            ...
            productAPI.caseinfoDel({ id: this.dialog.delete.id }).then(res => {
                if (res.code == 0) {
